...
class UserService:
    ...
    def remove_comic(self, chat_id: int, comic_title: str) -> bool:
        user = self.get_user_by_chat_id(chat_id)
        if user:
            user.comics = [comic for comic in user.comics if comic.title != comic_title]
            self.update_user(user)
            
    def get_comics(self, chat_id: int) -> List[Comic]:
        user = self.get_user_by_chat_id(chat_id)
        if user:
            return user.comics
        return []
